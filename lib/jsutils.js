...
var JSUtils = {
	 ...
	 * @param {Object} [ignoreFieldsReal] Properties to exclude from freezing
	 */
	deepFreeze: function (object, ignoreFieldsReal) {
		const ignoreFields = ignoreFieldsReal || {};
		...
		const self = this;
		Object.getOwnPropertyNames( object ).forEach( function ( propKey ) {
			const prop = object[propKey];

			// If the field is explicitly specified as ignored, skip it.
			if ( ignoreFields.hasOwnProperty( propKey ) ) {
				return;
			}

			// If the object is not an object, or is already frozen,
