...
const freezeExcludedProperties = {
	CHAR_REFS_RE: true,
	EVIL_URI_RE: true,
...
JSUtils.deepFreeze(SanitizerConstants, freezeExcludedProperties);
...
Sanitizer.prototype.onAny = function ( token ) {
	...
	env.log("trace/sanitizer", this.manager.pipelineId, function() { return JSON.stringify(token); });
	...
	{ // unknown tag -- convert to plain text
		...
		if ( origToken.name === 'section' ) {
			// Special handling for wikisource: hide section tags for now
			// TODO: Properly handle this. See
			// https://www.mediawiki.org/wiki/Parsoid/HTML_based_LST
			token = new SelfclosingTagTk('meta',
					[{k: 'typeof', v: 'mw:Placeholder'}],
					{
						src: token,
						tsr: origToken.tsr
					});
