 ...
 * @class
 *
 * Calls MW api.php to parse <infobox> to HTML
 *
 * @constructor
 * @param {MWParserEnvironment} env
 * @param {String} tagsrc
 ...
 * @param {String} title
 */
function InfoboxRequest( env, tagsrc, args, title ) {
	ApiRequest.call( this, env, null );
	...
	this.requestOptions = {
		...
		timeout: 16 * 1000, // 16 seconds (same as for action=parse)
		proxy: env.conf.wiki.apiProxyURI,
		headers: {
			'User-Agent': userAgent,
			'Connection': 'close'
...
util.inherits( InfoboxRequest, ApiRequest );

InfoboxRequest.prototype._handleJSON = function ( error, data ) {
	if ( data && data.infobox && data.infobox.text ) {
		this._processListeners( null, data.infobox.text['*'] );
	} else {
		this._processListeners( error, null );
	}
};
...
if (typeof module === "object") {
	...
	module.exports.InfoboxRequest = InfoboxRequest;
